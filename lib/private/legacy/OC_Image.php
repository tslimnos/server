...
class OC_Image implements \OCP\IImage {
	...
	protected const DEFAULT_MEMORY_LIMIT = 128 * 1024 * 1024;
	...
	private function checkImageMemory($width, $height) {
		$memory_limit = (int) $this->config->getAppValue('preview', 'memory_limit', (string) self::DEFAULT_MEMORY_LIMIT);
		if ($memory_limit <= 0) {
			// Not limited.
			return true;
		}
		...
		if ($width * $height * 4 > $memory_limit) {
			$this->logger->warning('Image size of ' . $width . 'x' . $height . ' would exceed allowed memory limit of ' . $memory_limit);
			return false;
		}
